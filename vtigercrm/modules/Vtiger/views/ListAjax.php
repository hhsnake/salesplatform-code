...
class Vtiger_ListAjax_View extends Vtiger_List_View {
	...
	public function searchAll(Vtiger_Request $request) {
		...
		$searchableModules = Vtiger_Module_Model::getSearchableModules();
                ...
                //SalesPlatform.ru begin
		$showModule = $request->get('showModule');
                if ($showModule) {
                    if ($page) {
                        $pagingModel->set('page', $page);
                    }
                    foreach ($searchableModules as $tableName => $vall) {
                        if ($showModule == $tableName) {
		...
		foreach ($matchingRecords as $module => $recordModelsList) {
			...
			$recordModelsList = array_keys($recordModelsList);
                        ...
                        //SalesPlatform.ru begin
			$recordModelsList = array_slice($recordModelsList, ($page - 1) * ($pageLimit - 1), $pageLimit);
			...
			$listViewModel->listViewHeaders = $listViewModel->getListViewHeaders(true);
                        //SalesPlatform.ru begin
			//$listViewModel->set('pageNumber', 1);
			$listViewModel->set('pageNumber', $page);
                        //SalesPlatform.ru end
		...
		$viewer->assign('SEARCH_VALUE', $searchValue);
                ...
                //SalesPlatform.ru begin
		$viewer->assign('PAGE_NUMBER', $page);
