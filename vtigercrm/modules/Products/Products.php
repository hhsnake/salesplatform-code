	...
	*/
	...
	function get_accounts($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		if($actions) {
			...
			if(in_array('ADD', $actions) && isPermitted($related_module,1, '') == 'yes') {
                ...
                // SalesPlatform.ru begin Modified query to Products
		$query = "SELECT vtiger_crmentity.crmid, vtiger_accountbillads.bill_city, vtiger_account.*, case when (vtiger_users.user_name not like \"\") then vtiger_users.user_name else vtiger_groups.groupname end as user_name, vtiger_crmentity.smownerid, vtiger_products.productname, vtiger_products.qty_per_unit, vtiger_products.unit_price, vtiger_products.expiry_date
			FROM vtiger_account
	...
	function get_contacts($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		if($actions) {
			...
			if(in_array('ADD', $actions) && isPermitted($related_module,1, '') == 'yes') {
                ...
                $query = "SELECT vtiger_contactdetails.*, vtiger_crmentity.crmid, case when (vtiger_users.user_name not like \"\") then vtiger_users.user_name else vtiger_groups.groupname end as user_name, vtiger_crmentity.smownerid, vtiger_products.productname, vtiger_products.qty_per_unit, vtiger_products.unit_price, vtiger_products.expiry_date,vtiger_account.accountname
	...
	function get_opportunities($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$userNameSql = getSqlForNameInDisplayFormat(array('first_name'=>
                ...
                // SalesPlatform.ru begin Modified query to Products
		$query = "SELECT vtiger_potential.*, vtiger_crmentity.crmid,
