		...
		<tr class="listViewEntries" data-id='{$LISTVIEW_ENTRY->getId()}' data-recordUrl='{$LISTVIEW_ENTRY->getDetailViewUrl()}' id="{$MODULE}_listView_row_{$smarty.foreach.listview.index+1}">
			...
			<td class="listViewEntryValue {$WIDTHTYPE}" data-field-type="{$LISTVIEW_HEADER->getFieldDataType()}" data-field-name="{$LISTVIEW_HEADER->getFieldName()}" nowrap>
				...
				{else if $LISTVIEW_HEADER->get('uitype') eq '72'}
					...
					{/if}
				{else}
                                        {* SalesPlatform begin localization added *}
                                        {vtranslate($LISTVIEW_ENTRY->get($LISTVIEW_HEADERNAME),$MODULE_NAME)}
			...
			<tr>
				<td>
					{assign var=SINGLE_MODULE value="SINGLE_$MODULE"}
                                        ...
                                        {vtranslate('LBL_NOT_FOUND')} {vtranslate($MODULE, $MODULE)} {vtranslate('LBL_FOUND')}.{if $IS_MODULE_EDITABLE} {vtranslate('LBL_CREATE')} <a href="{$MODULE_MODEL->getCreateRecordUrl()}">{vtranslate($SINGLE_MODULE, $MODULE)}</a>{/if}
