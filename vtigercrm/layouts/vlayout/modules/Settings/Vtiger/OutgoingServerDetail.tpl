		...
		<table class="table table-bordered table-condensed themeTableColor">
			...
			<tbody>
                                ...
                                <tr><td width="25%"><label class="muted pull-right marginRight10px">{vtranslate('LBL_SENDMAIL', $QUALIFIED_MODULE)}</label></td>
                                        <td style="border-left: none;"><span>{if $MODEL->isUseSendMailEnabled() neq 'false'}{vtranslate('LBL_YES', $QUALIFIED_MODULE)} {else}{vtranslate('LBL_NO', $QUALIFIED_MODULE)}{/if}</span></td></tr> 
				{* SalesPlatform.ru end *}
					...
					<td style="border-left: none;"><span>{$MODEL->get('server')}</span></td></tr>
                                ...
                                <tr><td><label class="muted pull-right marginRight10px">{vtranslate('LBL_SERVER_PORT', $QUALIFIED_MODULE)}</label></td>
                                        <td style="border-left: none;"><span>{$MODEL->get('server_port')}</span></td></tr>
                                {* SalesPlatform.ru end *}
				...
				<tr><td><label class="muted pull-right marginRight10px">{vtranslate('LBL_FROM_EMAIL', $QUALIFIED_MODULE)}</label></td>
					<td style="border-left: none;"><span>{$MODEL->get('from_email_field')}</span></td></tr>
                                ...
                                <tr><td><label class="muted pull-right marginRight10px">{vtranslate('LBL_FROM_NAME', $QUALIFIED_MODULE)}</label></td>
                                        <td style="border-left: none;"><span>{$MODEL->get('from_name')}</span></td></tr>
				{* SalesPlatform.ru end *}
					...
					<td style="border-left: none;"><span>{if $MODEL->isSmtpAuthEnabled() neq 'false'}{vtranslate('LBL_YES', $QUALIFIED_MODULE)} {else}{vtranslate('LBL_NO', $QUALIFIED_MODULE)}{/if}</span></td></tr>
                                ...
                                <tr><td><label class="muted pull-right marginRight10px">{vtranslate('LBL_CONNECTION_SECURITY', $QUALIFIED_MODULE)}</label></td>
                                     <td style="border-left: none;"><span>{if $MODEL->get('server_tls') eq 'tls'}{vtranslate('LBL_TLS', $QUALIFIED_MODULE)} {elseif $MODEL->get('server_tls') eq 'ssl'}{vtranslate('LBL_SSL', $QUALIFIED_MODULE)} {else} {vtranslate('LBL_NO_TLS', $QUALIFIED_MODULE)}{/if}</span></td></tr>
                                {* SalesPlatform.ru end *}
			</tbody>
		</table>
	</div>
