...
{if $MODULEMODEL and $MODULEMODEL->isActive() and $FIELD_VALUE}
    ...
    {if $PERMISSION}
        ...
        <a class="phoneField" style="color: #000 !important;" data-value="{$PHONE_NUMBER}" record="{$RECORD->getId()}" onclick="Vtiger_PBXManager_Js.registerPBXOutboundCall('{$PHONE_NUMBER}',{$RECORD->getId()})">{$FIELD_MODEL->get('fieldvalue')}</a>
    {else}
