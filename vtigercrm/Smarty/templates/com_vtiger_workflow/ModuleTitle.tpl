<table width="100%" cellspacing="0" cellpadding="5" border="0" class="settingsSelUITopLine"><tbody>
	<tr>
		<td width="50" valign="top" rowspan="2">
			<img width="48" height="48" border="0" src="{'settingsWorkflow.png'|@vtiger_imageurl:$THEME}" alt="{$PAGE_NAME}" title="{$PAGE_NAME}"/>
		</td>
		<td valign="bottom" class="heading2">
                        {* SalesPlatform.ru begin : localization added *}
			<b><a href="index.php?module=Settings&amp;action=index&amp;parenttab=Settings">{'Settings'|getTranslatedString:'Settings'}</a> >
				<a href="index.php?module={$module->name}&amp;action=workflowlist&amp;parenttab=Settings">{$MODULE_NAME|getTranslatedString:$MODULE_NAME}</a> > {$PAGE_NAME} </b>
			{* <b><a href="index.php?module=Settings&amp;action=index&amp;parenttab=Settings">Settings</a> >  *}
			{* 	<a href="index.php?module={$module->name}&amp;action=workflowlist&amp;parenttab=Settings">{$MODULE_NAME}</a> > {$PAGE_NAME} </b> *}
                        {* SalesPlatform.ru end *}
		</td>
	</tr>
	<tr>
		<td valign="top" class="small">{$PAGE_TITLE}</td>
	</tr>
</tbody></table>
<br>
