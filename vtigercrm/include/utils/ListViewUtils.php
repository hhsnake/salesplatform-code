			...
			foreach ($focus->list_fields as $name => $tableinfo) {
				...
				if ($is_admin == true || $profileGlobalPermission[1] == 0 || $profileGlobalPermission[2] == 0 || in_array($fieldname, $field) || $fieldname == '' || ($name == 'Close' && $module == 'Calendar')) {
					if ($fieldname == '') {
						...
						if($name == "Subject" && $tmp_value == "")
							$tmp_value = getTranslatedString("{No Subject}", "Emails");
						// SalesPlatform.ru end
...
// SalesPlatform.ru begin
					...
					$tmp_arr = array("entityid"=>$entity_id,"prodname"=>"".stripslashes(decode_html(nl2br($slashes_temp_val)))."","unitprice" => "$unitprice", "qtyinstk"=>"$qty_stock","taxstring"=>"$tax_str","rowid"=>"$row_id","desc"=>stripslashes($slashes_desc),"subprod_ids"=>"$sub_det","prod_code"=>"$order_code","prod_sheet"=>"$prod_sheet");
...
// SalesPlatform.ru begin
					$value = '<a href="javascript:window.close();" id=\'popup_product_' . $entity_id . '\' onclick=\'set_return_inventory("' . $entity_id . '", "' . decode_html(nl2br($slashes_temp_val)) . '", "' . $unitprice . '", "' . $qty_stock . '","' . $tax_str . '","' . $row_id . '","' . $slashes_desc . '","' . $sub_det . '","'.$order_code.'","'.$prod_sheet.'");\' vt_prod_arr=\'' . $prod_arr . '\' >' . textlength_check($temp_val) . '</a>';
...
// SalesPlatform.ru end
					...
					$prod_arr = Zend_Json::encode($tmp_arr);
// SalesPlatform.ru begin
					$value = '<a href="javascript:window.close();" id=\'popup_product_' . $entity_id . '\' onclick=\'set_return_inventory_po("' . $entity_id . '", "' . decode_html(nl2br($slashes_temp_val)) . '", "' . $unitprice . '", "' . $tax_str . '","' . $row_id . '","' .decode_html($slashes_desc) .'","'.$sub_det.'","'.$order_code.'","'.$prod_sheet.'"); \'  vt_prod_arr=\''.$prod_arr.'\' >'.textlength_check($temp_val).'</a>';
...
// SalesPlatform.ru begin
					$prod_sheet = '';
					$tmp_arr = array("entityid" => $entity_id, "prodname" => "" . stripslashes(decode_html(nl2br($slashes_temp_val))) . "", "unitprice" => "$unitprice", "taxstring" => "$tax_str", "rowid" => "$row_id", "desc" => "$slashes_desc","prod_code"=>"$order_code","prod_sheet"=>"$prod_sheet");
...
// SalesPlatform.ru begin
					$value = '<a href="javascript:window.close();" id=\'popup_product_' . $entity_id . '\' onclick=\'set_return_inventory("' . $entity_id . '", "' . decode_html(nl2br($slashes_temp_val)) . '", "' . $unitprice . '", "' . $tax_str . '","' . $row_id . '","' . $slashes_desc . '","'.$order_code.'","'.$prod_sheet.'");\'  vt_prod_arr=\''.$prod_arr.'\' >'.textlength_check($temp_val).'</a>';
...
// SalesPlatform.ru end
				...
				elseif ($popuptype == "set_return_emails") {
					if ($module == 'Accounts') {
						...
						$value = '<a href="javascript:select_return_emails('.$entity_id.',\''.decode_html($slashes_name).'\',\''.$module.'\');" id = ' . $count . '>'.textlength_check($name).'</a>';
						// SalesPlatform.ru end
						...
						$value = '<a href="javascript:select_return_emails('.$entity_id.',\''.decode_html($slashes_name).'\',\''.$module.'\');" id = ' . $count . '>'.$name.'</a>';
						// SalesPlatform.ru end
					}else {
						$name = getFullNameFromQResult($list_result, $list_result_count, $module);
						$emailaddress = $adb->query_result($list_result, $list_result_count, "email1");
...
// SalesPlatform.ru begin
                        $value = '<a href="javascript:window.close();" onclick=\'set_return("' . $entity_id . '", "' . nl2br(decode_html($slashes_temp_val)) .'","'.$_REQUEST['user_field'].'");\'';
...
function getListQuery($module, $where = '') {
	...
	switch ($module) {
		...
		Case "Potentials":
                        // SalesPlatform.ru begin Modified query to Potentials (select relatedlists)
			//Query modified to sort by assigned to
			$query = "SELECT vtiger_crmentity.crmid, vtiger_crmentity.smownerid,
...
// SalesPlatform.ru begin: Added select possibility for contacts activities
	elseif($currentmodule == "Calendar" && $returnmodule == "Contacts")
	{
		$reltable = "vtiger_cntactivityrel";
