...
// This function code created from function export($type) {} from export.php
function get_filtered_ids($module){
    global $adb;
    $focus = 0;
    if (!empty($module)) {
    ...
    $params = array();
    // Hack Query: adding crmid field
    $query_body = explode('SELECT', trim($query), 2);
    $query = "SELECT vtiger_crmentity.crmid, ".$query_body[1];   // $query_body[0] is SELECT word
    $result = $adb->pquery($query, $params, true, "Error exporting $module: "."<BR>$query");
    $count = $adb->num_rows($result);
    $ids = array();
    for ($i = 0; $i < $count; $i++) {
