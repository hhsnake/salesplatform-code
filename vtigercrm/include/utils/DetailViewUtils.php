...
function getDetailViewOutputHtml($uitype, $fieldname, $fieldlabel, $col_fields, $generatedtype, $tabid='', $module='') {
	...
	if ($uitype == '10') {
		...
		for ($j = 0; $j < $noofpickrows; $j++) {
			...
			$pickListValue_translated = to_html(getTranslatedString( html_entity_decode($pickListValue) ));
			// SalesPlatform.ru end
			$pickListValue = to_html($pickListValue);
			// SalesPlatform.ru begin
			//$options[] = array(getTranslatedString($pickListValue),$pickListValue,$chk_val );	
			$options[] = array($pickListValue_translated,$pickListValue,$chk_val );
			// SalesPlatform.ru end
		}
...
//				else
				// To fit image while keeping ratio
				if($pro_image_width  > 450)
					$label_fld[] ='<img src="'.$imagepath_array[0].$image_id_array[0]."_".$image_array[0].'" border="0" width="450">';
				elseif($pro_image_height > 300)
					$label_fld[] ='<img src="'.$imagepath_array[0].$image_id_array[0]."_".$image_array[0].'" border="0" height="300">';
				else
				// SalesPlatform.ru end
...
function getDetailAssociatedProducts($module, $focus) {
	...
	for ($i = 1; $i <= $num_rows; $i++) {
		...
		if ($discount_percent != 'NULL' && $discount_percent != '') {
			...
			$discount_info_message = "$discount_percent % ".$app_strings['of_string']." ".
...
// SalesPlatform.ru end
	...
	if ($focus->column_fields['hdnDiscountPercent'] != '0') {
		...
		$final_discount_info = $focus->column_fields['hdnDiscountPercent'] . " % ".$app_strings['of_string']." ".CurrencyField::convertToUserFormat($netTotal, null, true).
