...
function getblock_fieldlist($block_array/* SalesPlatform.ru begin */, $stop_blockname = '' /* SalesPlatform.ru end */)
{
	$list='';$z=0;
	...
	if($field_count != 0)
	{
		for($i=0;$i<$field_count;$i++,$z++)
		{
			$blockname = $block_array[$i]['blockname'];
                        ...
                        if (!empty($stop_blockname) && strcmp($stop_blockname, $blockname) == 0) {
                            break;
                        }
			...
			if(($z == 0 || $z%2 == 0) &&($i == ($field_count-1)))
				$list .= '<td colspan="3" width="20%" class="dvtCellInfo">'.getTranslatedString($data).'</td>';
			else
				$list .= '<td width="20%" class="dvtCellInfo">'.getTranslatedString($data).'</td>';
...
// SalesPlatform.ru begin
function getblock_fieldlist_managed($block_array, $start_blockname, $cols = 2, $id_field = '')
{
	$list = '';
        $z = 0;
	$field_count = count($block_array);
	...
	if ($field_count != 0) {
                ...
                for ($i = 0; $i < $field_count; $i++) {
                        // Skip ID field data
                        if (!empty($id_field) && strstr($block_array[$i]['fieldlabel'], $id_field)) {
                                continue;
                        }
                        // Skip some data
                        $blockname = $block_array[$i]['blockname'];
                        if (!empty($start_blockname)) {
                            if (!$process) {
                                if (strcmp($start_blockname, $blockname) == 0) {
                                    $process = true;
                                } else {
                                    continue;
                                }
                        ...
                        if (!$header_done) {
                                $list .= '<tr>';
                                for($j = $i; $j < ($cols + $i); $j++) {
                                    $list .= '<td class="dvtCellLabel" width="'.$width.'%" align="left">'.$block_array[$j]['fieldlabel'].'</td>';
                                }
                                $list .= '</tr>';
			...
			if(($z == 0 || ($z % $cols) == 0) &&($i == ($field_count-1)))
				$list .= '<td colspan="'.($field_count-1).'" width="'.$width.'%" class="dvtCellInfo">'.getTranslatedString($data).'</td>';
			else
				$list .= '<td width="'.$width.'%" class="dvtCellInfo">'.getTranslatedString($data).'</td>';
                        $z++;
...
function get_field_value($invinfo, $fieldlabel) {
        $nooffields = count($invinfo);
        for ($i = 0; $i < $nooffields; $i++) {
                if (strcmp($invinfo[$i]['fieldlabel'], $fieldlabel) == 0) {
                        return $invinfo[$i]['fieldvalue'];
                }
        }
}

function getblock_fieldlistedit($block_array, $block, $id_field_name = "", $invinfo = array(), $id_field_label = "") {
	...
	if($nooffields!='')
	{
		...
		for($i=0;$i<$nooffields;$i++) {
                        if (!empty($id_field_name) && (strcmp($id_field_name, $header_arr[$i]['fielddata']) == 0)) {
                            $id_ix = $i;
                            $list .= "<td  class='detailedViewHeader' >$id_field_label</td>";
                        } else {
		...
		for($j=0;$j<$noofdata;$j++)
		{
			...
			for($i=0;$i<$nooffields;$i++)
			{
				$data =$data_arr[$j][$i]['fielddata'];
                                if (!empty($id_ix) && $id_ix == $i) {
                                    $list .= "<td><input type=\"text\" name=\"".$id_field_name.$data."\" value=\"".((int)get_field_value($invinfo, 'productid'.$data))."\" style=\"width: 40px;\"></td>";
                                    $ids .= ($data."_");
                                    continue;
                                }
				if($data == '')
			...
			$list .='</tr>';
                ...
                if (!empty($ids)) {
                    $list .= "<input type=\"hidden\" name=\"ids\" value=\"".$ids."\">";
                }
